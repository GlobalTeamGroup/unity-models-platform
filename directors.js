...
document.getElementById('casting-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        statusEl.textContent = '✅ Кастинг успешно опубликован! Он теперь доступен на платформе.';
        statusEl.className = 'status-msg text-center success';
        e.target.reset();
    } catch (err) {
        console.error('Error posting casting:', err);
        statusEl.textContent = '❌ Не удалось опубликовать кастинг. Проверьте консоль или попробуйте снова.';
