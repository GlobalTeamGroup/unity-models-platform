...
document.getElementById('login-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (error || !data) {
            statusEl.textContent = '❌ Профиль не найден. Пожалуйста, сначала зарегистрируйтесь через @unitymodelsbot.';
        ...
        document.getElementById('model-name').textContent = data.first_name || 'Модель';
...
document.getElementById('profile-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        statusEl.textContent = '✅ Профиль успешно обновлен!';
        statusEl.className = 'status-msg text-center success';
    } catch (err) {
        console.error('Error updating profile:', err);
        statusEl.textContent = '❌ Ошибка при сохранении профиля.';
        statusEl.className = 'status-msg text-center error';
    } finally {
        btn.disabled = false;
    }
...
document.getElementById('btn-upload').addEventListener('click', async (e) => {
    ...
    try {
        ...
        statusEl.textContent = '✅ Фото успешно загружены!';
        statusEl.className = 'status-msg text-center success';
        fileInput.value = ''; // clear
    } catch (err) {
        console.error('Error uploading:', err);
        statusEl.textContent = '❌ Ошибка загрузки. Убедитесь, что корзина portfolios создана.';
        statusEl.className = 'status-msg text-center error';
    } finally {
...
document.getElementById('calendar-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        statusEl.textContent = '✅ Статус обновлен!';
        statusEl.className = 'status-msg text-center success';
    } catch (err) {
        console.error('Error updating calendar:', err);
        statusEl.textContent = '❌ Ошибка при обновлении статуса.';
        statusEl.className = 'status-msg text-center error';
    } finally {
        btn.disabled = false;
    }
